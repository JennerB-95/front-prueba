<template>
  ...
  >
    <p class="custom-font text-h6 font-weight-bold mb-5">Editar empleado</p>
...
const handleGet = async () => {
  try {
    const response = await axios.get(`api/v1/employee/profile/${depId}/`);
    code.value.value = response.data.code;
    first_name.value.value = response.data.first_name;
    last_name.value.value = response.data.last_name;
    birthday.value.value = response.data.birthday;
    department.value.value = response.data.department;
    unit.value.value = response.data.department.unit;
  } catch (error) {
...
const submit = handleSubmit(async (values) => {
  try {
    const response = await axios.patch(`api/v1/employee/${depId}/`, {
      code: values.code,
    ...
    Swal.fire("Empleado!", "El registro ha sido editado", "success");
